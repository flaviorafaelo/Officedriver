...
export function bootstrapApp() {
	...
	async function loginSuccess() {
		...
		try {
			if (_startupUrl) {
				...
				if (!curFrame) curFrame = millennium.browser.createFrame()
	...
	let currentLoginCallback: (host: string, userName: string, password: string) => Promise<void>
	...
	async function defaultFinishLogin(host: string, userName: string, password: string) {
		...
		if (!millennium.browser.inLogin) {
			...
			try {
				...
				// if (licenceName === "loja" || licenceName === "retag")//legacgacy
	...
	async function loginClick() {
		try {
			await currentLoginCallback($('#hostname').val() as string, ($("#username").val() as string).toUpperCase(), ($("#password").val() as string).toUpperCase())
		} catch (e) {
			millennium.browser.notify(e, "danger")
			throw e
		}
	...
	function setupUI() {
		...
		window.onerror = (_errorMsg, _source, _lineno, _colno, errorObj) => {
			if (errorObj instanceof Error) {
				if (errorObj.message.substring(0, 5) === "show:") {
					millennium.browser.notify(errorObj.message.substring(5), "danger")
				}
			}
		}
	...
	async function startup(startupPage: string) {
		if (!millennium.browser.isEmbedded) {
			const versions = await millennium.browser.getVersion()
			$(".versionOmnichannel").text(versions["omni"] ?? "")
			$(".versionMillennium").text(versions["millennium"] ?? "")
